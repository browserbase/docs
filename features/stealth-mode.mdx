...
<Note>
  - `browserListQuery` takes precedence on `browsers` <br />
  - `operatingSystems` set to `ios` or `android` requires `devices` to include `"mobile"`{" "} <br />
  - `locales` accepts 10 values maximum <br />
</Note>
