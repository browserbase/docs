...
<Note>
  A Session created through the API needs to be started in the following 15min,
  otherwise it will moved automatically to the `COMPLETED` state.
</Note>
        ...
        function fetchDownloads() {
            try {
              const response = await fetch(
                `https://connect.browserbase.com/v1/sessions/${sessionId}/downloads`, {
                  method: 'GET',
                  headers: {
