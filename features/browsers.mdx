...
The viewport, user agents and header configuration is handled by the
...
1. Use the [Session API](/api-reference/list-all-sessions) to create a new
   session.
2. Then, use the [Session API debug endpoint](/api-reference/list-all-sessions)
   to get the `debuggerUrl`.
3. Finally, load an iFrame with the `debuggerUrl` or `debuggerFullscreenUrl`
   (_recommended_) as the src and `sandbox="allow-same-origin allow-scripts"`.
   The sandbox permissions are necessary for now as we continue to customize our
   Chrome Dev Tools implementation:
...
2. Then, use the
   [Session API create session endpoint](/api-reference/list-all-sessions) to
