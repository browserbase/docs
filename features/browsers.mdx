...
1. Use the Session API to [create a new session](/api-reference/create-a-session).
2. Then, use the [Session API debug endpoint](/api-reference/retrieve-a-session-debug-connection-urls)
   to get the `debuggerFullscreenUrl`.
3. Finally, open the `debuggerFullscreenUrl` URL in Chrome to start taking action on the session.
